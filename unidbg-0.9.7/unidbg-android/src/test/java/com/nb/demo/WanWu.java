...
public class WanWu extends AbstractJni {
    ...
    public WanWu() {
        ...
        emulator = AndroidEmulatorBuilder.for64Bit().setProcessName("com.wanwu").build();
        // 2.获取内存对象（可以操作内存）
        ...
        vm = emulator.createDalvikVM(new File("unidbg-0.9.7/apks/ww/wanwu.apk"));
        vm.setJni(this); // 后期补环境会用，把要补的环境，写在当前这个类中，执行这个代码即可，但是必须继承AbstractJni
        //vm.setVerbose(true); //是否展示调用过程的细节

        // 5.加载so文件
        DalvikModule dm = vm.loadLibrary(new File("unidbg-0.9.7/apks/ww/libkiwi.so"), false);   // 以后会动
    ...
    public void sign() {
        ...
        DvmClass CheckSignUtil = vm.resolveClass("com/kiwi/sdk/Kiwi");

        // 2 找到类中的方法--》固定写法
        // 方法名(参数签名)返回值签名
        String method = "server_to_local(Ljava/lang/String;)Ljava/lang/String;";
        // 3 执行这个方法，传入参数
        //第一个参数是：设备对象
        // 第二个参数是：方法
        // 第三个参数往后的是：方法要传的参数,传参数的具体方式，下小结讲

        StringObject stringObject = new StringObject(vm,"product_api");
